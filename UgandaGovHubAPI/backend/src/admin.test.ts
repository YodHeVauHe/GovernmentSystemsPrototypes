...
async function main() {
  ...
  await withPostgresTestDb(async partialDb => {
    ...
    `);
    await ensureAdminSchema(partialDb);

    const rows = await partialDb.prepare(`
      SELECT table_name as name
      FROM information_schema.tables
      WHERE table_schema = 'public'
        AND table_name IN ('access_requests', 'audit_logs')
      ORDER BY table_name
    `).all();
    assert.deepEqual(rows, [{ name: 'access_requests' }, { name: 'audit_logs' }]);
  });
}
