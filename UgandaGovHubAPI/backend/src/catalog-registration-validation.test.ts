...
const apiOwner = {
  id: 'usr-catalog-validation-owner',
  full_name: 'Catalog Validation Owner',
  email: 'catalog.validation.owner@example.go.ug',
...
function createCatalogRegistrationValidationDb() {
  let apiInserts = 0;
  ...
  const db = {
    async query(sql: string, params: unknown[] = []) {
      ...
      throw new Error(`Unexpected SQL in catalog registration validation test: ${normalizedSql}`);
    },
    async exec() {},
...
async function main() {
  const fake = createCatalogRegistrationValidationDb();
  const app = await startApp(fake.db);

  try {
    const registration = await request(app.baseUrl, '/api/catalog', {
      method: 'POST',
      headers: { authorization: 'Bearer catalog-registration-validation-session' },
      body: JSON.stringify({
        name: { localized: 'Object Name API' },
    ...
    assert.match(registration.body.error, /name/i);
...
main().then(() => {
  console.log('catalog registration validation tests passed');
}).catch(error => {
  console.error(error);
  process.exit(1);
});
