...
const apiOwner = {
  id: 'usr-catalog-update-validation-owner',
  full_name: 'Catalog Update Validation Owner',
  email: 'catalog.update.validation.owner@example.go.ug',
...
function createCatalogUpdateValidationDb() {
  const api = {
    id: 'api-catalog-validation',
    ...
    openapi_spec_path: '/openapi/api-catalog-validation.yaml',
  ...
  const db = {
    async query(sql: string, params: unknown[] = []) {
      ...
      if (normalizedSql.includes('SELECT id FROM apis WHERE id = $1 AND owning_mda_id = $2')) {
        assert.deepEqual(params, ['api-catalog-validation', 'mda-catalog-owner']);
      ...
      throw new Error(`Unexpected SQL in catalog update validation test: ${normalizedSql}`);
    },
    async exec() {},
...
async function main() {
  const fake = createCatalogUpdateValidationDb();
  const app = await startApp(fake.db);

  try {
    const update = await request(app.baseUrl, '/api/catalog/api-catalog-validation', {
      method: 'PATCH',
      headers: { authorization: 'Bearer catalog-update-validation-session' },
      body: JSON.stringify({
    ...
    assert.match(update.body.error, /description/i);
...
main().then(() => {
  console.log('catalog update validation tests passed');
}).catch(error => {
  console.error(error);
  process.exit(1);
});
