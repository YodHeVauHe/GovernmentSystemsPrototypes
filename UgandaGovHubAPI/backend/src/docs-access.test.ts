...
async function main() {
  await withPostgresTestDb(async db => {
    ...
    await db.prepare(`
      INSERT INTO access_requests (
        id, consumer_user_id, consumer_type, api_id, status, api_key_hash, api_key_status, api_key_expires_at, api_key_revoked_at
      ) VALUES (?, ?, 'user', ?, ?, ?, ?, ?, ?)
    `).run('req-stale-revoked', 'usr-stale-revoked', 'api-restricted', 'APPROVED', 'hashed_stale_key', 'ACTIVE', null, '2026-05-22T10:00:00.000Z');
    ...
    const staleRevokedDeveloper = { id: 'usr-stale-revoked', status: 'APPROVED' as const, role: 'developer' as const, mda_id: null };
    ...
    assert.equal(deniedCode(await canViewApiDocs(db, staleRevokedDeveloper, 'api-restricted')), 'FORBIDDEN');
    ...
    assert.equal(deniedCode(await canDownloadOpenApiAsset(db, staleRevokedDeveloper, '/openapi/restricted.yaml')), 'FORBIDDEN');
    ...
    assert.deepEqual((await listVisibleDocsApis(db, staleRevokedDeveloper)).map(api => api.id), ['api-auth', 'api-public']);
